...
mod=brm( bf(N_total|rate(Ndays)~scaled_IMD_100+(1|SITE)+(scaled_IMD_100|CAMP),
...
newdata=dataNB |> 
  ...
  mutate(SITE="RB00",
         Ndays=1) |> 
  distinct()  |>  
  add_epred_draws(mod,re_formula=~(scaled_IMD_100|CAMP))

dataNB |>  
  ggplot()+
  geom_point(aes(meanIMD_100m,N_total/Ndays),col="black")+
  stat_lineribbon(data=newdata,aes(x=meanIMD_100m,y=.epred),
                  .width=c(0.001,0.95),fill="grey",alpha=0.5)+
  scale_x_continuous("mean Imperviousness Density (100 m radius around site)")+
  scale_y_continuous("proportion of black-legged individuals")+
